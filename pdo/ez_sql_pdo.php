<?php
    ...
    */
    ...
    class ezSQL_pdo extends ezSQLcore
    {
        ...
        var $chunk_position=0;
        var $query_ident=false;
        ...
        */
        ...
        function escape($str)
        {
            ...
            switch (gettype($str))
            {
                case 'string' : $str = $this->dbh->quote($str);
        ...
        */
        ...
        function catch_error()
        {
            ...
            if ( isset($err_array[1]) && $err_array[1] != 25)
            {
                ...
                if($this->show_errors)
                {
                    ...
                    //exit;
                    trigger_error("$error_str \nSQL:$this->last_query\nDBG:".$this->myDebugDumpParams(), E_USER_WARNING);
                    exit;
                }
        ...
        function myDebugDumpParams()
        {
            if ( ! isset($this->dbh) || ! $this->dbh ) return false;
            if ( ! isset($this->sth) || ! $this->sth ) return false;
            ob_start();
        ...
        function get_chunk()
        {
            $query=$vars=$chunk_size=false;
            foreach(func_get_args() as $arg)
            {
                if (! $chunk_size AND is_numeric($arg))
                {
                    $chunk_size = $arg;
                }
                elseif (! $vars and is_array($arg))
                {
                    $vars = $arg;
                }
                elseif (! $query AND !is_array($arg) AND !is_numeric($arg) )
                {
                    if ( preg_match("/^(select)\s+/i",$arg) )
                    {
                        $query = $arg;
                    }
                }
            }
            if (! $query OR ! $chunk_size)
            {
                trigger_error("Insufficient args passed (no query ($query) or no chunk size ($chunk_size))" , E_USER_WARNING);
                exit;
            }
//            echo "query $query, $chunk_size \n";
            $query_ident = md5($query.print_r($vars));
//            echo "query_ident $query_ident \n";
//            echo "this->query_ident $this->query_ident \n";

            // check if its a new query or an repeated chunk
            if($query_ident <> $this->$query_ident)
            {
                // its a new query
//                echo "New Query \n";
                $this->$query_ident = $query_ident;
                $this->chunk_position = 0;
            }
            $query .= " LIMIT $chunk_size OFFSET $this->chunk_position ";
            $this->chunk_position = $this->chunk_position + $chunk_size;
//            echo "query $query \n";
//            echo "this->chunk_position $this->chunk_position \n";
        ...
        function query($query,$vars=false)
        {
            ...
            $this->func_call = "\$db->query(\"$query\",\"".print_r($vars)."\")";

            // Keep track of the last query for debug..
            $this->last_query = $query.print_r($vars);
            ...
            if ( $cache = $this->get_cache($query,$vars) )
            {
            ...
            if ($vars) {
                ...
                // Perform the query
                $this->sth = $this->sth->execute($query);
                ...
                else
                {
                    ...
                    for ( $i=0 ; $i < $col_count ; $i++ )
                    {
                        ...
                        if ( $meta = $this->sth->getColumnMeta($i) )
                        {
            ...
            $this->store_cache($query,$is_insert,$vars);
