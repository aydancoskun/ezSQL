    ...
    */
    ...
    class ezSQLcore
    {
        ...
        */
        ...
        function store_cache($query,$is_insert,$vars=false)
        {
            $query.= print_r($vars);
        ...
        */
        ...
        function get_cache($query, $vars=false)
        {
            $query.= print_r($vars);
            // The would be cache file for this query
            $cache_file = $this->cache_dir.'/'.md5($query);
        ...
        */
        ...
        /**********************************************************************
        * Creates a SET string from an associative array or object and escapes
        * all values.
        *
        *  Usage:
        *
        ...
        */
        ...
        function get_set($params)
        {
            ...
            if( !is_array( $params ) )
            {
                $this->register_error( 'get_set() parameter invalid in '.__FILE__.' on line '.__LINE__);
                return;
            }
            $sql = array();
        ...
        /**********************************************************************
        * The command pair get_key_set() and get_val_set() creates an
        *
        * 'INSERT INTO users () VALUES ()' string from an associative array or
        *
        * object and escapes all values.
        *
        *  Usage:
        *
        *     $db_data = array('login'=>'jv','email'=>'jv@vip.ie', 'user_id' => 1, 'created' => 'NOW()');
        *
        *     $db->query("INSERT INTO users (".get_key_set($db_data).") VALUES (".$db->get_val_set($db_data).")");
        */

        function get_val_set($params)
        {
            if(is_object($params))
            {
                $params = (array) $params;
            ...
            if( !is_array( $params ) )
            {
                $this->register_error( 'get_key_set() parameter invalid in '.__FILE__.' on line '.__LINE__);
                return;
            }
            $sql = array();
        ...
        function get_key_set($params)
        {
            ...
            if( !is_array( $params ) )
            {
                $this->register_error( 'get_val_set() parameter invalid in '.__FILE__.' on line '.__LINE__);
                return;
            }
            $sql = array();
            foreach ( $params as $field => $val )
        ...
        function get_idx_set($params)
        {
            ...
            if( !is_array( $params ) )
            {
                $this->register_error( 'get_idx_set() parameter invalid in '.__FILE__.' on line '.__LINE__);
                return;
            }
            $sql = array();
            foreach ( $params as $field => $val )
